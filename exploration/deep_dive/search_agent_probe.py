...
logger = logging.getLogger("search_agent_probe")
...
def parse_args() -> argparse.Namespace:
    cfg = load_config()
    p = argparse.ArgumentParser(description="Agent-based probe for Foundry WebSearchTool behavior.")
...
def _write_outputs(run_id: str, payload: dict[str, Any]) -> tuple[Path, Path]:
    ...
    json_path = out_dir / f"search_agent_probe_{run_id}.json"
    md_path = out_dir / f"search_agent_probe_{run_id}.md"
    json_path.write_text(json.dumps(payload, indent=2), encoding="utf-8")

    lines: list[str] = []
    lines.append(f"# Search Agent Probe ({run_id})")
    lines.append("")
    lines.append("## Inputs")
    lines.append("")
...
def main() -> None:
    ...
    out_dir = Path(__file__).resolve().parent / "output"
    log_path = out_dir / f"search_agent_probe_{run_id}.log"
    ...
    with DefaultAzureCredential() as credential, AIProjectClient(endpoint=cfg.project_endpoint, credential=credential) as project_client, project_client.get_openai_client() as openai_client:
        ...
        agent_name = f"{cfg.agent_name_prefix}-search-{args.model.lower().replace('.', '-').replace('_', '-')}-{run_stamp}"
        agent = project_client.agents.create_version(
            ...
            description="Exploration agent for Foundry WebSearchTool.",
        )
