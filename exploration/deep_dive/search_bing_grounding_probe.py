...
logger = logging.getLogger("search_bing_grounding_probe")
...
def parse_args() -> argparse.Namespace:
    cfg = load_config()
    p = argparse.ArgumentParser(description="Agent-based probe for Foundry Bing grounding behavior.")
    ...
    p.add_argument("--connection-id", default="", help="Explicit project connection id for GroundingWithBingSearch.")
    p.add_argument("--market", default="en-US", help="Bing market, e.g. en-US.")
    p.add_argument("--set-lang", default="en-US", help="Language used for Bing UI strings.")
    p.add_argument("--count", type=int, default=5, help="Number of Bing results to request.")
    p.add_argument("--freshness", default="", help="Optional Bing freshness filter, e.g. Day, Week, Month.")
    return p.parse_args()
...
def _discover_bing_connection_id(project_client: AIProjectClient) -> str:
    matches: list[tuple[bool, str]] = []
    for conn in project_client.connections.list():
        raw = conn.as_dict() if hasattr(conn, "as_dict") else {}
        conn_type = str(getattr(conn, "type", raw.get("type", "")) or "")
        meta_type = str(raw.get("metadata", {}).get("type", "") or "")
        if conn_type == "GroundingWithBingSearch" or meta_type == "bing_grounding":
            conn_id = str(getattr(conn, "id", raw.get("id", "")) or "")
            is_default = bool(getattr(conn, "is_default", raw.get("isDefault", False)))
            matches.append((is_default, conn_id))
    if not matches:
        raise SystemExit("No GroundingWithBingSearch project connection found.")
...
def _write_outputs(run_id: str, payload: dict[str, Any]) -> tuple[Path, Path]:
    ...
    json_path = out_dir / f"search_bing_grounding_probe_{run_id}.json"
    md_path = out_dir / f"search_bing_grounding_probe_{run_id}.md"
    json_path.write_text(json.dumps(payload, indent=2), encoding="utf-8")

    lines: list[str] = []
    lines.append(f"# Search Bing Grounding Probe ({run_id})")
    lines.append("")
    lines.append("## Inputs")
    lines.append("")
    ...
    lines.append("")
    lines.append("| Case | Run | Success | Status | Latency (ms) | Citations | x-request-id | Detail |")
    lines.append("|---|---:|---|---:|---:|---:|---|---|")
    ...
    lines.append("")
    lines.append("| Case | Run | Status | APIM Request ID | x-request-id | x-ms-region |")
    lines.append("|---|---:|---:|---|---|---|")
    for rec in payload["records"]:
        if rec["success"]:
            continue
        headers = rec.get("response_headers") or {}
        lines.append(
            f"| {rec['case_name']} | {rec['run_index']} | {rec['status_code'] or '-'} | "
...
def main() -> None:
    ...
    out_dir = Path(__file__).resolve().parent / "output"
    out_dir.mkdir(parents=True, exist_ok=True)
    log_path = out_dir / f"search_bing_grounding_probe_{run_id}.log"
    ...
    total_calls = len(selected_cases) * args.runs
    progress = 0
    ...
    with DefaultAzureCredential() as credential, AIProjectClient(endpoint=cfg.project_endpoint, credential=credential) as project_client, project_client.get_openai_client() as openai_client:
        ...
        connection_id = args.connection_id or _discover_bing_connection_id(project_client)
        logger.info("bing_connection_id=%s", connection_id)
        ...
        if args.freshness:
            config_kwargs["freshness"] = args.freshness
        grounding_config = BingGroundingSearchConfiguration(**config_kwargs)
        grounding_params = BingGroundingSearchToolParameters(search_configurations=[grounding_config])
        grounding_tool = BingGroundingTool(bing_grounding=grounding_params)
        ...
        agent_name = f"{cfg.agent_name_prefix}-bing-{args.model.lower().replace('.', '-').replace('_', '-')}-{run_stamp}"
        agent = project_client.agents.create_version(
            agent_name=agent_name,
            definition=PromptAgentDefinition(
                model=args.model,
                instructions="You are a research assistant. Use Bing grounding when needed and cite sources.",
                tools=[grounding_tool],
            ),
            description="Exploration agent for Foundry Bing grounding.",
        )
                ...
                for run_index in range(1, args.runs + 1):
                    ...
                    try:
                        ...
                        if rec.success:
                            ...
                            logger.warning(
                                "FAIL  progress=%s/%s case=%s run=%s status=%s latency_ms=%s error=%s",
                                progress,
                                total_calls,
                                case.name,
                                run_index,
